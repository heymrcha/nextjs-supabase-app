...
```bash
# 필수 패키지 (현재 프로젝트에는 아직 설치되지 않음 — 폼 작업 시작 시 설치)
npm install react-hook-form @hookform/resolvers zod

# 고급 기능을 위한 추가 패키지 (선택적)
npm install use-debounce react-error-boundary

# 이 문서가 기준으로 삼는 버전 (2026-09 기준 최신)
# - react-hook-form: ^7.88.0
# - @hookform/resolvers: ^5.9.1
# - zod: ^4.6.5  ← Zod 4 API 기준. z.string().email() 같은 v3 문법은 deprecated
```
...
// 공통 폼 타입 정의
export type FormState<T extends z.ZodType> = {
  success: boolean
  message?: string
...
// 폼 훅 공통 인터페이스
export interface FormHookProps<T extends z.ZodType> {
  schema: T
...
// 🚀 재사용 가능한 기본 스키마 컴포넌트
// Zod 4: .email()은 deprecated → z.email() 최상위 함수 사용
...
export function useOptimizedForm<T extends z.ZodType>(
  schema: T,
  defaultValues?: Partial<z.infer<T>>
) {
  // 🚀 스키마 메모이제이션
