...
- **TailwindCSS v3.4**: 유틸리티 기반 CSS 프레임워크 (`tailwind.config.ts` + `@tailwind` 지시자 방식. v4의 `@import "tailwindcss"`·`@theme` 문법은 사용하지 않음)
- **shadcn/ui**: Radix UI 기반 컴포넌트 라이브러리 (new-york style)
- **next-themes**: 다크모드 지원
- **tailwindcss-animate**: 애니메이션 플러그인 (`tailwind.config.ts`의 `plugins`에 등록)
- **CSS Variables**: 동적 테마 시스템
- **prettier-plugin-tailwindcss**: 자동 클래스 정렬 (선택, 현재 미설치)
...
```tsx
// tailwind.config.ts의 plugins에 tailwindcss-animate가 등록되어 있어 별도 import 불필요
