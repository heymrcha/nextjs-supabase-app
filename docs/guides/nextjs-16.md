...
// next.config.ts에서 typedRoutes: true 설정 필요 (16부터 experimental 아님)
...
Next.js 16부터 Turbopack이 `dev`와 `build` 모두의 기본 번들러입니다. 별도 플래그 없이 동작하며, 설정은 `experimental.turbo`가 아닌 최상위 `turbopack` 키에 둡니다.
...
Next.js 16부터 `middleware.ts`는 `proxy.ts`로 이름이 바뀌었습니다. 런타임은 Node.js로 고정되며 Edge Runtime은 지원하지 않습니다. 이 프로젝트의 인증 세션 갱신 로직은 루트 `proxy.ts` → `lib/supabase/proxy.ts`에 있습니다.
...
기존 프로젝트 마이그레이션은 codemod로 처리할 수 있습니다.
