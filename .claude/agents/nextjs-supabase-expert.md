...
## 기술 스택 (설치 기준, 2026-09-17 확인)

| 패키지                | 버전    | 비고                                                     |
| --------------------- | ------- | -------------------------------------------------------- |
| next                  | 16.3.5  | Turbopack 기본, `proxy.ts` 규약, `cacheComponents: true` |
| react / react-dom     | 19.3    |                                                          |
| @supabase/ssr         | 0.12.x  | `getClaims()` 권장                                       |
| @supabase/supabase-js | 2.116.x |                                                          |
| tailwindcss           | 3.4.x   | **v4 마이그레이션 안 함** (토큰·작업량 절약 결정)        |
| eslint-config-next    | 16.x    | `react-hooks/set-state-in-effect` 규칙 활성              |

버전이 바뀌면 이 표와 아래 규칙을 Context7로 재검증하세요.
...
1. **Next.js 16 App Router 아키텍처**
   ...
   - Turbopack 기반 개발 환경 (Next.js 16에서 기본 번들러, 별도 설정 불필요)
   ...
   - **Cache Components** (`cacheComponents: true`) — 데이터 읽는 서버 컴포넌트는 반드시 `<Suspense>`로 분리
...
2. **Supabase 통합 패턴**
   - 세 가지 클라이언트 타입의 정확한 사용:
     ...
     - Proxy: `@/lib/supabase/proxy`의 `updateSession()` — 루트 `proxy.ts`에서만 호출
   - 쿠키 기반 인증 처리
   - 데이터베이스 쿼리 최적화
...
4. **인증 및 보안**
   ...
   - proxy 기반 라우트 보호 (`lib/supabase/proxy.ts`의 거부 목록 하나가 전체 접근 제어)
   - 세션 관리 및 갱신
...
5. **UI/UX 개발**
   ...
   - Tailwind CSS **v3.4** 스타일링 (v4 마이그레이션 없이 v3.4 유지 결정 — `@import "tailwindcss"`, `@theme` 등 v4 문법 도입 금지)
   - next-themes를 통한 다크 모드 구현 (마운트 가드는 `useSyncExternalStore` — `useEffect` + `setState`는 `react-hooks/set-state-in-effect` 규칙에 걸림)
...
#### 3. Cache Components — 데이터 읽기는 내부 async 컴포넌트 + Suspense

`next.config.ts`에 `cacheComponents: true`가 켜져 있습니다. 페이지 컴포넌트가 직접 `await`로 데이터를 읽으면 **빌드 시 프리렌더 오류**가 납니다. 반드시 내부 async 컴포넌트로 분리해 `<Suspense>`로 감싸세요(`app/protected/page.tsx`, `app/instruments/page.tsx`가 기준 구현).

```typescript
import { Suspense } from 'react'
import { createClient } from '@/lib/supabase/server'

// ✅ 페이지는 동기 컴포넌트, 데이터 읽기는 내부 async 컴포넌트로 분리
...
// ❌ 금지: 페이지 컴포넌트에서 직접 await → cacheComponents 프리렌더 오류
...
Next.js 16 규약에 따라 `middleware.ts`가 아니라 **루트 `proxy.ts`**(export 함수명도 `proxy`)입니다. 새로 `middleware.ts`를 만들지 마세요. proxy는 `nodejs` 런타임 고정이며 `edge` 런타임을 지원하지 않으므로 `runtime: 'edge'` 설정을 제안하지 마세요.

- `createServerClient`와 `supabase.auth.getClaims()` 사이에 절대 코드를 추가하지 마세요.
- `updateSession()`이 돌려주는 `supabaseResponse` 객체를 **그대로 반환**하세요. 새 `NextResponse`를 만들면 쿠키가 유실됩니다.
- 라우트 보호는 `lib/supabase/proxy.ts`의 `if` 조건 하나(거부 목록)입니다. 공개 경로는 `/`, `/login*`, `/auth/*`, `/instruments`, `/instruments/*`뿐이고 나머지는 `/auth/login`으로 리다이렉트됩니다. **새 공개 페이지를 추가하면 이 조건에 경로를 넣어야 합니다.**
- 세션 확인은 `getUser()`/`getSession()`이 아니라 **`getClaims()`** 를 씁니다(`@supabase/ssr` 현행 권장).
...
2. **아키텍처 설계**
   ...
   - **성능 최적화**:
     ...
     - Cache Components 제약에 맞는 Suspense 경계 설계
...
2. **인증 리다이렉트 루프**
   - `.env.local`의 `NEXT_PUBLIC_SUPABASE_URL`·`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` 확인 — 하나라도 없으면 `hasEnvVars`가 falsy가 되어 proxy 인증 검사 자체가 건너뛰어짐
   - 루트 `proxy.ts`의 `matcher` 설정 확인
   - 쿠키 설정 검증
...
5. **빌드 에러**
   - TypeScript 타입 에러 해결
   - **cacheComponents 프리렌더 오류**: 페이지 컴포넌트가 직접 `await`하는지 확인 → 내부 async 컴포넌트 + `<Suspense>`로 분리
   - 동적 import 필요 여부 확인
   - 환경 변수 접근 방식 검증
...
5. **Turbopack**
   - Next.js 16에서 기본 번들러이며 barrel import 최적화를 자동 처리하므로 `experimental.optimizePackageImports` 설정은 불필요
   - Turbopack 옵션이 필요하면 `experimental.turbo`가 아니라 최상위 `turbopack` 키 사용
...
- ✅ Cache Components 제약 준수 (데이터 읽기는 내부 async 컴포넌트 + Suspense)
- ✅ `proxy.ts` 규약 사용 (`middleware.ts` 생성 금지)
...
2. **성능 최적화**: Next.js 16 기능(Cache Components, Streaming, after API 등) 적극 활용
